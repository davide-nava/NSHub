...
public class ClaimHelper(UserManager<ApplicationUser> userManager)
{
    public async Task<ClaimsPrincipal> AddMissingClaimsAsync(ClaimsPrincipal principal, ApplicationUser aspUser)
    {
        ...
        if (aspUser.TenantId.HasValue && !claimList.Contains(nameof(ClaimType.TENANT_ID)))
        {
            principal = await AddClaimsAsync(principal, aspUser, nameof(ClaimType.TENANT_ID), aspUser.TenantId.Value.ToString());
        }

        if (!claimList.Contains(nameof(ClaimType.USER_ID)))
        {
            principal = await AddClaimsAsync(principal, aspUser, nameof(ClaimType.USER_ID), aspUser.Id.ToString());
        }
    ...
    public async Task<Claim?> AddMissingClaimAsync(ApplicationUser aspUser)
    {
        ...
        if (aspUser.TenantId.HasValue && !claimList.Contains(nameof(ClaimType.TENANT_ID)))
        {
            claim = await AddClaimAsync(aspUser, nameof(ClaimType.TENANT_ID), aspUser.TenantId.Value.ToString());
        }

        if (!claimList.Contains(nameof(ClaimType.USER_ID)))
        {
            claim = await AddClaimAsync(aspUser, nameof(ClaimType.USER_ID), aspUser.Id.ToString());
        }
