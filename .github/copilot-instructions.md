# GitHub Copilot Instructions for NSHub

## 1. Architectural Philosophy & Layer Boundaries

NSHub is an enterprise-grade, portable mini ERP / business management system built on .NET 11 and Clean Architecture. Strict unidirectional dependencies MUST always be respected:

- `NSHub.Domain`: Pure enterprise domain model. No external infrastructure dependencies. Encapsulated entities, private setters, invariant enforcement.
- `NSHub.Application`: Core business use cases following CQRS via MediatR. Validations through FluentValidation pipeline behaviors. Defines repository and service abstractions (`IApplicationDbContext`, `ICurrentUserService`, `IIdentityService`).
- `NSHub.Infrastructure`: EF Core with SQLite, persistence, and external service integrations.
- `NSHub.Api`: ASP.NET Core Web API presentation layer with endpoints protected by authorization.
